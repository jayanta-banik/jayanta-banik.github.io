...
(function () {
  ...
  function dna(ctx, w, h, t, m) {
    ...
    for (var i = 0; i < N; i++) {
      ...
      var y1 = cy + amp * Math.sin(ai),
        y2 = cy - amp * Math.sin(ai);
  ...
  function graphInit(w, h) {
    ...
    var nodes = P.map(function (p, i) {
      return { x: p[0] * w, y: p[1] * h, s: i * 1.7 };
    });
  ...
  function graph(ctx, w, h, t, m, st) {
    ...
    nodes.forEach(function (n, i) {
      ...
      ctx.beginPath();
      ctx.arc(n.dx, n.dy, hot ? 5 : near ? 3.6 : 2.6, 0, 7);
      ctx.fill();
      if (hot) {
        ctx.strokeStyle = 'rgba(14,163,107,.5)';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.arc(n.dx, n.dy, 7 + Math.sin(t * 4) * 1.5, 0, 7);
        ctx.stroke();
      }
    });
    ctx.fillStyle = INK;
  ...
  function moe(ctx, w, h, t, m, st) {
    ...
    if (cyc !== st.cyc) {
      // new gating each cycle
      st.cyc = cyc;
      var ws = st.ex.map(function () {
        return Math.random();
      });
      var sum = ws.reduce(function (a, b) {
        return a + b;
      }, 0);
    ...
    var hovE = -1;
    st.ex.forEach(function (e, i) {
      if (m.over && m.x > e.x - 4 && Math.abs(m.y - (e.y + 6)) < 9) hovE = i;
    });
    ...
    ctx.fillStyle = BLU;
    ctx.beginPath();
    ctx.arc(inx, iny, 4, 0, 7);
    ctx.fill();
    // router
    ctx.strokeStyle = 'rgba(30,41,59,.3)';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(inx, iny);
    ctx.lineTo(rx, ry);
  ...
  var SIGN_COL = BLU,
    LLM_COL = PUR;
  var SIGN_FONT = 'bold 9px monospace',
    LLM_FONT = 'italic 9px monospace';
  function handInit() {
    var poses = [
      // curl per [thumb, index, middle, ring, pinky]
    ...
    final.forEach(function (tk, i) {
      (tk.src === 'sign' ? signIdx : llmIdx).push(i);
    });
    return {
      poses: poses,
      rest: rest,
  ...
  function layoutTokens(ctx, final, cx, bottomY, maxW) {
    ...
    final.forEach(function (tk, i) {
      ...
      if (lineW[li] + add > maxW && lines[li].length) {
        lines.push([]);
        lineW.push(0);
        li++;
        add = wWidth;
      }
      lines[li].push({ i: i, w: wWidth });
      lineW[li] += add;
    });
    var lh = 12,
      startY = bottomY - (lines.length - 1) * lh,
      pos = {};
  ...
  function hand(ctx, w, h, t, m, st) {
    ctx.clearRect(0, 0, w, h);
    var nSteps = st.order.length,
      interval = m.over ? 0.9 : 1.6,
      holdTime = 2.4;
    ...
    if (m.clicks !== st.lastClick) {
      st.lastClick = m.clicks;
      if (st.gen < nSteps) step();
      st.stepT = t;
    }
    if (st.gen < nSteps) {
      if (t - st.stepT > interval) step();
    } else if (t - st.stepT > holdTime) {
      st.gen = 0;
      st.at = {};
      st.poseTgt = st.rest;
      st.stepT = t;
    ...
    var bases = [
      ...
      [wx - 6, wy - 30, -Math.PI / 2 - 0.1, [15, 11, 9], st.cur[2]],
    ...
    bases.forEach(function (b, i) {
      finger(ctx, b[0], b[1], b[2], b[3], b[4], cols[i]);
    });
    ...
    st.final.forEach(function (tk, i) {
      ...
      if (tk.src === 'sign') {
        // pop in
        var e = easeOut(age / 0.28),
          s = 0.65 + 0.35 * e;
        ctx.save();
        ctx.globalAlpha = e;
        ctx.translate(p.x, p.y);
        ctx.scale(s, s);
        ctx.fillStyle = SIGN_COL;
        ctx.font = SIGN_FONT;
  ...
  function emo(ctx, w, h, t, m, st) {
    ...
    ctx.lineWidth = 1.6;
    ctx.beginPath();
    ctx.moveTo(fx - 13, fy - 11 + st.brow * 3);
    ctx.lineTo(fx - 4, fy - 11 - st.brow * 3);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(fx + 4, fy - 11 - st.brow * 3);
    ctx.lineTo(fx + 13, fy - 11 + st.brow * 3);
    ctx.stroke();
    ...
    st.list.forEach(function (em, i) {
      ...
      ctx.fill();
      ctx.fillStyle = em.c;
      rr(ctx, bx, y, Math.max(2, bw * st.vals[i]), 5, 2);
      ctx.fill();
    });
    ctx.fillStyle = INK;
    ctx.font = '8px monospace';
  ...
  function lstm(ctx, w, h, t, m, st) {
    ...
    hidY.forEach(function (hy) {
      ctx.strokeStyle = 'rgba(224,138,0,.75)';
      ctx.lineWidth = 1.2;
      ctx.beginPath();
      ctx.arc(hx, hy - 9, 5.5, Math.PI * 0.15, Math.PI * 1.7);
      ctx.stroke();
      var aa = Math.PI * 1.7,
        ax = hx + 5.5 * Math.cos(aa),
        ay = hy - 9 + 5.5 * Math.sin(aa);
    ...
    var ph = (t * speed * 0.7) % 1,
      px,
      py;
    if (ph < 0.5) {
      var k = ph / 0.5;
      px = lerp(ix, hx, k);
      py = lerp(inY[1], hidY[0], k);
    } else {
      var k2 = (ph - 0.5) / 0.5;
      px = lerp(hx, ox, k2);
      py = lerp(hidY[0], outY, k2);
    }
    ctx.fillStyle = PNK;
    ctx.beginPath();
    ...
    ctx.fillStyle = AMB;
    hidY.forEach(function (hy) {
      rr(ctx, hx - 5, hy - 5, 10, 10, 3);
      ctx.fill();
    });
    ctx.fillStyle = GRN;
    ctx.beginPath();
    ctx.arc(ox, outY, 3, 0, 7);
    ctx.fill();
    ctx.fillStyle = INK;
    ctx.font = '7px monospace';
    ctx.textAlign = 'center';
    ctx.fillText('LSTM', hx, h - 5);
    ctx.textAlign = 'left';
    ...
    var codeX = 90,
      y0 = 24,
      lh = 12;
    ctx.strokeStyle = 'rgba(14,163,107,.35)';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(ox + 4, outY);
    ctx.lineTo(codeX - 4, y0 - 5);
    ctx.stroke();

    // ----- right: character-by-character code generation -----
    var cps = m.over ? 32 : 16;
    if (st.start === 0) st.start = t;
    var total = st.code.reduce(function (a, l) {
      return a + l.length + 1;
    }, 0);
    var revealed = Math.floor((t - st.start) * cps);
    if (revealed > total + cps * 1.3) {
      st.start = t;
      revealed = 0;
    } // pause, then loop
    ctx.font = '8px monospace';
    ...
    for (var li = 0; li < st.code.length; li++) {
      ...
      if (revealed >= idx && revealed <= idx + line.length && Math.floor(t * 2.5) % 2 === 0) {
        ...
        ctx.fillStyle = GRN;
        ctx.fillRect(codeX + cw + 1, yy - 7, 4, 8); // blinking caret
      }
      idx += line.length + 1;
      yy += lh;
  ...
  function sva(ctx, w, h, t, m, st) {
    ...
    var u0 = t * speed * 0.6;
    function yOf(u) {
      return midY - A * Math.sin(u * 0.8 + seed) - A * 0.35 * Math.sin(u * 1.9 + seed * 1.3);
    }
    function xOf(u) {
      return plotX + ((u - u0) / span) * plotW;
    }
    ...
    ctx.strokeStyle = 'rgba(43,127,255,.9)';
    ctx.lineWidth = 1.6;
    ctx.beginPath();
    ...
    anchors.forEach(function (p) {
      ctx.strokeStyle = 'rgba(43,127,255,.4)';
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(p.x, midY);
      ctx.lineTo(p.x, p.y);
    ...
    if (qErr > 3) {
      ...
      ctx.lineWidth = 1.4;
      ctx.beginPath();
      ctx.arc(xOf(qu), yOf(qu), 6 + Math.sin(t * 5) * 2, 0, 7);
      ctx.stroke();
      ctx.fillStyle = AMB;
      ctx.beginPath();
      ctx.arc(xOf(qu), yOf(qu), 2, 0, 7);
      ctx.fill();
    }
